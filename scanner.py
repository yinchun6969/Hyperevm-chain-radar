...
def _decode_symbol(result):
    ...
    try:
        h=result[2:]
        if len(h)==64:
            return bytes.fromhex(h).rstrip(b'\0').decode('utf-8','ignore')
        b=bytes.fromhex(h); off=int.from_bytes(b[:32],'big'); n=int.from_bytes(b[off:off+32],'big')
        return b[off+32:off+32+n].decode('utf-8','ignore')
    except Exception:return ''

class Scanner:
    def __init__(self):
        urls=[os.getenv('HYPEREVM_RPC_URL',DEFAULT_RPC)]+[x.strip() for x in os.getenv('HYPEREVM_RPC_URLS','').split(',') if x.strip()]
        self.rpc=RPCPool(urls,int(os.getenv('RPC_FAILBACK_SEC','300')))
        self.store=Store(os.getenv('DB_PATH',str(__import__('pathlib').Path(__file__).with_name('radar.db'))))
        ...
        self.backfill=int(os.getenv('START_BACKFILL_BLOCKS','20')); self.poll=float(os.getenv('POLL_SECONDS','1'))
        self.alert=float(os.getenv('ALERT_USD','250000'))
        self.meta={}
    ...
    def process_pool_created(self,l,b):
        ...
        if not x:return
        self.store.add_pool(x['pool'],'HyperSwap V3',x['token0'],x['token1'],x['fee'],b,l.get('transactionHash'))
        s0,_=self.token_meta(x['token0']); s1,_=self.token_meta(x['token1'])
    ...
    def process_pool_activity(self,l,b):
        x=hyperswap_v3.parse_activity(l)
        if not x:return
        pool=(l.get('address') or '').lower()
        r=self.store.db.execute('SELECT token0,token1 FROM pools WHERE address=?',(pool,)).fetchone()
        if not r:return
        t0,t1=r; u0,s0=self.amount_usd(t0,x['amount0']); u1,s1=self.amount_usd(t1,x['amount1'])
        vals=[v for v in (u0,u1) if v is not None]; usd=sum(vals) if len(vals)==2 else (vals[0] if vals else None)
        kind=x['kind']; direction=None
        if kind=='SWAP':
            # Direction is from token0 perspective. Pool-positive token0 means trader sells token0.
            direction='SELL_TOKEN0' if x['amount0']>0 else 'BUY_TOKEN0' if x['amount0']<0 else 'SWAP'
        actor=topic_addr((l.get('topics') or [None,None])[1]) if len(l.get('topics') or [])>1 else None
        self._save(ts=int(time.time()),block_number=b,tx_hash=l.get('transactionHash'),source='hyperevm',kind=kind,protocol='HyperSwap V3',token=t0,symbol=f'{s0 or t0[:8]}/{s1 or t1[:8]}',actor=actor,pool=pool,direction=direction,usd=usd,details=x)
        if kind in ('ADD_LP','REMOVE_LP'):self.rug.on_lp(pool,kind,usd or 0,t0,s0)
    def process_tx(self,tx,b):
        cw=corewriter.parse_tx(tx)
        if cw:
            self._save(ts=int(time.time()),block_number=b,tx_hash=tx.get('hash'),source='hyperevm',kind='COREWRITER',protocol='HyperCore',actor=(tx.get('from') or '').lower(),direction='EVM_TO_CORE_ACTION',details=cw)
        # Best-effort HYPE Core->EVM system transaction detection.
        if (tx.get('from') or '').lower()==HYPE_SYSTEM and (tx.get('to') or '').lower()!=HYPE_SYSTEM:
            raw=int(tx.get('value','0x0'),16); px=float(self.store.kv_get('hype_usd','0') or 0)
            self._save(ts=int(time.time()),block_number=b,tx_hash=tx.get('hash'),source='hyperevm',kind='CORE_EVM_TRANSFER',protocol='HyperCore',token=HYPE_SYSTEM,symbol='HYPE',actor=(tx.get('to') or '').lower(),direction='CORE_TO_EVM',usd=raw/1e18*px if px else None,details={'raw':raw,'confidence':'best_effort_system_tx'})
    def scan_block(self,b):
        block=self.call('eth_getBlockByNumber',[_hex(b),True],20) or {}
        bt=classify_block(block); self.store.kv_set('block_type',bt)
        for tx in block.get('transactions') or []:self.process_tx(tx,b)
        # One-block queries stay comfortably inside HyperEVM's documented <=50 block log range.
        ...
        for l in logs:
            self.process_core_transfer(l,b); self.process_pool_created(l,b)
        ...
        if pools:
            for i in range(0,len(pools),100):
                try:
                    ...
                    for l in ls:self.process_pool_activity(l,b)
    ...
    def run(self,stop):
        ...
        while not stop.is_set():
            try:
                ...
                while last<tip and not stop.is_set():
                    last+=1; self.scan_block(last); self.store.kv_set('scanner_cursor',last)
                stop.wait(self.poll)
