...
class Scanner:
    ...
    def process_core_transfer(self,l,b):
        ...
        self._save(ts=int(time.time()),block_number=b,tx_hash=l.get('transactionHash'),source='hyperevm',kind='CORE_EVM_TRANSFER',protocol='HyperCore',token=token,symbol=sym,actor=x.get('actor'),direction=x['direction'],usd=usd,details=x)
    ...
    def process_pool_activity(self,l,b):
        ...
        if not r:return
        t0,t1=r;u0,s0=self.amount_usd(t0,x['amount0']);u1,s1=self.amount_usd(t1,x['amount1']);vals=[v for v in (u0,u1) if v is not None];usd=sum(vals) if len(vals)==2 else (vals[0] if vals else None);kind=x['kind'];direction=None
        if kind=='SWAP':direction='SELL_TOKEN0' if x['amount0']>0 else 'BUY_TOKEN0' if x['amount0']<0 else 'SWAP'
    ...
    def scan_block(self,b):
        block=self.call('eth_getBlockByNumber',[_hex(b),True],20) or {};self.store.kv_set('block_type',classify_block(block))
